...
class PIDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        int_error = self.Kw.dot(self.integ_error) + e

        controller_velocity = target_velocity + (self.Kp).dot(e) + (self.Ki).dot(int_error) + (self.Kd).dot(e_dot)

        self.integ_error = int_error
