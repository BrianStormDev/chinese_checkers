...
class LinearTrajectory(Trajectory):
    def __init__(self, start_position, goal_position, total_time):
        ...
        self.test_pos = self.start_position - np.array([0, 0.5, 0]) 
        self.mid_point_pos = 0.5 * self.acceleration * (self.total_time / 2) ** 2 + self.start_position
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            pos = -0.5 * self.acceleration * new_time ** 2 + self.v_max * new_time + self.mid_point_pos
        result = np.hstack((pos, self.desired_orientation))
        return result
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            init_vel = self.target_velocity(self.total_time / 2)[:3]
            linear_vel = self.acceleration * -1 * (time - self.total_time / 2) + init_vel
        result = np.hstack((linear_vel, np.zeros(3)))
        return result
...
class CircularTrajectory(Trajectory):
    ...
    def get_theta(self, time): 
        if time <= self.total_time / 2.0:
            ...
            new_time = time - self.total_time / 2 
            mid_point_angular = 0.5 * self.angular_acceleration * (self.total_time / 2) ** 2
            theta = -0.5 * self.angular_acceleration * new_time ** 2 + self.angular_v_max * new_time + mid_point_angular
        return theta
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            new_time = time - self.total_time / 2
            init_vel = self.angular_acceleration * (self.total_time / 2)
            theta_dot = -1 * self.angular_acceleration * new_time + init_vel
        # print(f'DEBUG: theta = {theta}. theta dot = {theta_dot}')
        vel_d = np.ndarray.flatten(self.radius * theta_dot * np.array([-np.sin(theta), np.cos(theta), 0]))
        print(f'DEBUG: Returning target vel. Shape = {vel_d.shape}')
...
if __name__ == '__main__':
    ...
    path = LinearTrajectory(np.array([0, 0, 0]), np.array([0.1, 0.1, 0.1]), 10)
